...
if ($in{'delete'}) {
	...
	@mods = grep { !/^(tcp|udp|icmp|multiport|mac|limit|owner|state|tos)$/ } map { $_->[1] } @{$rule->{'m'}};
	$rule->{'cmt'} = $in{'cmt'};
	if ($in{'jump'} eq '*') {
	...
	if ($rule->{'chain'} eq 'INPUT' || $rule->{'chain'} eq 'OUTPUT' ||
	    $rule->{'chain'} eq 'DROP') {
	...
	if (&parse_mode("proto", $rule, "p")) {
		$in{'proto'} || &error($text{'save_eproto'});
		$rule->{'p'}->[1] = $in{'proto'};
		if (!$rule->{'p'}->[0]) {
			$proto = $in{'proto'};
			push(@mods, $in{'proto'})
